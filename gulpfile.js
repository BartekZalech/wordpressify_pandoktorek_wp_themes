...
const adminJS = ['./src/assets/admin/js/**'];
const footerJSLibraries = ['./src/assets/libraries/**'];
/* -------------------------------------------------------------------------------------------------
Installation Tasks
-------------------------------------------------------------------------------------------------- */
...
function adminScripts() {
  return src(adminJS)
    .pipe(plumber({ errorHandler: onError }))
    .pipe(
      dest('./build/wordpress/wp-content/themes/' + themeName + '/admin/js')
    );
}

function footerScriptsLibraries() {
  return src(footerJSLibraries)
    .pipe(plumber({ errorHandler: onError }))
    .pipe(
      dest('./build/wordpress/wp-content/themes/' + themeName + '/libraries')
    );
}
